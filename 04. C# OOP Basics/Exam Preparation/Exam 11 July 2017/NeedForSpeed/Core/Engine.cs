...
public class Engine
{
    ...
    private void DistributeCommands(List<string> commandArgs)
    {
        ...
        switch (command)
        {
            ...
            case "check":
                OutputWriteLine(this.manager.Check(int.Parse(commandArgs[1])));
                break;
            ...
            case "start":
                OutputWrite(this.manager.Start(int.Parse(commandArgs[1])));
                break;
    ...
    private void OutputWriteLine(string message) => Console.WriteLine(message);

    private void OutputWrite(string message) => Console.Write(message);
