...
namespace SIS.Services.UserCookieServices
{
    public class UserCookieService : IUserCookieService
    {
        ...
        private const string UserCookieKey = ".auth-cakes";

        public HttpCookie GetUserCookie(string username)
        {
            var cookieContent = EncryptString(username, EncryptKey);

            var cookie = new HttpCookie(UserCookieKey, cookieContent);
        ...
        {
            if (!cookies.ContainsCookie(UserCookieKey))
            {
                return null;
            }

            var cookie = cookies.GetCookie(UserCookieKey);
